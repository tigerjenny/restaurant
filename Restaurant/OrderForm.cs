    ...
    public partial class OrderForm : Form
    {
        ...
        */
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < listView1.Items.Count; i++)
            {
                if (listView1.Items[i].SubItems[0].Text == textBox1.Text)
                {
                    ...
                    i--;
                    //listView1.Items[i].SubItems[2].Text = Convert.ToString(Convert.ToInt32(listView1.Items[i].SubItems[2].Text) + Convert.ToInt32(textBox3.Text)); 
                }
            }
            ...
            if (textBox3.Text != "0")
            {
                ...
                lvi.SubItems.Add(Convert.ToString(num + Convert.ToInt32(textBox3.Text)));
                lvi.SubItems.Add(Convert.ToString(Convert.ToInt32(textBox2.Text) * (num + Convert.ToInt32(textBox3.Text))));
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("請點餐", "注意", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
            else
            {
                SumMomey();
            ...
            Integrated Security=True;Connect Timeout=30");
                SqlCommand cmd = new SqlCommand("SELECT * FROM vip", cn);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataSet ds = new DataSet();
                da.Fill(ds);
                DataTable dt = ds.Tables[0];
                ...
                try
                {
                    ...
                    {
                        if (number == dt.Rows[i]["phone"].ToString())
                        {
                            summeal = Convert.ToInt32(summeal * 0.9);
                            Pf2.label5.Text = "顧客: " + dt.Rows[i]["v_name"].ToString() + " 先生/女士";
                        }
                    }
                }
                catch
